...
class Webuntis extends utils.Adapter {
        ...
        if (this.config.baseUrl == "") {
            ...
            if (this.config.anonymous) {
                if (this.config.class == "") {
                    ...
                    const untis = new APIWebUntis.WebUntisAnonymousAuth(
                        this.config.school,
                        this.config.baseUrl
                    );
                ...
                if (this.config.username == "") {
                    ...
                    if ((this.config.login_method == "PasswordLogin")) {
                        ...
                        untis = new APIWebUntis.WebUntisSecretAuth(
                            this.config.school,
                            this.config.username,
                            this.config.client_secret,
                            this.config.baseUrl
        ...
        if (this.config.anonymous) {
            ...
            untis
                ...
                .then(async () => {
                    ...
                    untis
                        .getTimetableFor(new Date(), this.class_id, APIWebUntis.TYPES.CLASS)
                        .then(async (timetable) => {
                            ...
                            if (timetable.length > 0) {
                                ...
                                await untis
                                    ...
                                    .catch(async (error) => {
                                        this.log.error(
                                            "Cannot read Timetable data from 0 - possible block by scool"
                                        );
                            ...
                            untis
                                ...
                                .catch(async (error) => {
                                    this.log.error(
                                        "Cannot read Timetable data from +1 - possible block by scool"
                                    );
                                    this.log.debug(error);
                                });
                        });
            ...
            untis
                ...
                .then(async () => {
                    ...
                    untis
                        ...
                        .catch(async (error) => {
                            this.log.info(
                                "Cannot read Message Center - possible block by scool"
                            );
                            this.log.debug(error);
                        });
    ...
    async setTimeTable(timetable: Lesson[], dayindex: number): Promise<void> {
        ...
        await this.setStateAsync(
            dayindex + ".timetable-date",
            this.timetableDate.toString(),
            true
        );
        ...
        for (const element of timetable) {
            ...
            await this.setObjectNotExistsAsync(
                dayindex + "." + index.toString() + ".teacher",
                {
                    type: "state",
                    common: {
                        name: "teacher",
        ...
        if (index > 0) {
            ...
            await this.setStateAsync(
                dayindex + ".minTime",
                APIWebUntis.convertUntisTime(minTime, this.timetableDate).toString(),
                true
            );
            ...
            await this.setStateAsync(
                dayindex + ".maxTime",
                APIWebUntis.convertUntisTime(maxTime, this.timetableDate).toString(),
                true
            );
    ...
    private async deleteOldTimetableObject(
        dayindex: number,
        index: number
    ): Promise<void> {
        this.log.debug(
            "Object search in deleteOldTimetableObject for: " + index.toString()
        );

        const delObject = await this.getObjectAsync(
            dayindex + "." + index.toString() + ".name"
        );
    ...
    private getMillisecondsToNextFullHour(): number {
        ...
        const nextHour = new Date(
            ...
            5,
            0
        ); // add 5 seconds to ensure we are in the next hour
